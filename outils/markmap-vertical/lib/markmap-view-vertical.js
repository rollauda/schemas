/* markmap-view 0.18.12 modifié (option direction "TB") — GÉNÉRÉ par patch.mjs, ne pas éditer. */
  ...
  class Markmap {
    ...
    async renderData(originData) {
      ...
      mmGEnter.attr("transform", (d) => {
        ...
        return `translate(${isTB ? originRect.x + originRect.width / 2 - d.state.rect.width / 2 : originRect.x + originRect.width - d.state.rect.width},${originRect.y + originRect.height - d.state.rect.height})`;
      });
      ...
      this.transition(mmLineMerge).attr("x1", -1).attr("x2", (d) => d.state.rect.width + 2).attr("stroke", (d) => color(d)).attr("stroke-width", isTB ? 0 : lineWidth);
      const mmCircleExit = mmGExit.selectAll(
        childSelector("circle")
      );
      this.transition(mmCircleExit).attr("r", 0).attr("stroke-width", 0);
      mmCircleMerge.attr("cx", (d) => isTB ? d.state.rect.width / 2 : d.state.rect.width).attr("cy", (d) => d.state.rect.height + lineWidth(d) / 2);
